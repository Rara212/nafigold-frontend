...
export default function DashboardSidebar({ sidebarData }) {
  return (
    <div className="flex min-h-screen py-8 px-5 bg-white lg:w-1/5">
      {" "}
      <div className="flex-auto  h-full">
        <div className="flex flex-col overflow-y-auto justify-between gap-32">
          <div className="flex justify-center">
            {/* <img
              className="flex my-auto rounded-md w-[118px] h-[113px] "
              src="/assets/logo.png"
              alt="logo"
            /> */}
            <h3 className="font-title text-4xl  text-[#CEAE5E] ">Nafi Gold</h3>
          </div>
...
function DashboardItem({ item }) {
  ...
  const active = path === pathname ? "active" : null;
  const I = icon;
  return (
    <NavLink className={active}>
      <div className={` flex gap-5  text-[20px] px-24 text-[#1e1e1c]`}>
        <div>{label}</div>
        <I />
      </div>
    </NavLink>
